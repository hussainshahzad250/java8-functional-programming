...
public class StreamFilterExample {
	...
	public static void main(String[] args) {
		...
		PersonRepository.getAllPersons().stream().filter(p1.and(p2))
				// .filter(p2)
				.collect(Collectors.toList()).forEach(System.out::println);

		/*
		 * List<Person> l1 = PersonRepository.getAllPersons() .stream() .filter(per ->
		 * per.getHeight() >= 140) .collect(Collectors.toList()); l1.forEach(System.out
		 * :: println);
		 */
