...
public class StreamFlatMapExample {
	...
	static List<String> hobbies(List<Person> personsList) {
		return personsList.stream().map(Person::getHobbies).flatMap(List::stream).collect(Collectors.toList());
	}

	static List<String> distinctHobbies(List<Person> personsList) {
		return personsList.stream().map(Person::getHobbies).flatMap(List::stream).sorted().distinct()
				.collect(Collectors.toList());
	}

	static long countOfHobbies(List<Person> personsList) {
		return personsList.stream().map(Person::getHobbies).flatMap(List::stream).distinct().count();
	}
